...
def grid_search(
    ...
    for model, model_kwargs in tqdm(zip(cluster_models, model_kwargs_list)):
        for parameters in tqdm(ParameterGrid(model_kwargs)):
            labels = assign_cluster_labels(data_to_cluster, model, parameters)
            avg_silhouette_score = calculate_silhouette_avg(data_to_cluster, labels)
    ...
    if highlight:
        results = results.style.highlight_max(
            subset=["avg_silhouette_score"], color="lightgreen", axis=0
        )
    return results
